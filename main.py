...
class MainUI(QtWidgets.QMainWindow, main_window_class):
    ...
    def __init__(self, *args):
        ...
        def change_channel() : 
            randomChannel = random.randint(2,78)
            print("==================== Hopping to " + str(randomChannel) + "=============================")
            pk = CRTPPacket()
            pk.port = 15
            pk.channel = 3
            pk.data =  [0x03, 5, randomChannel]
            #pk.port = 0xff
            #pk.data = (5,100)
            self.cf.send_packet(pk)
            time.sleep(.5)
            self.cf.open_link("radio://0/" + str(randomChannel) + "/2M")

        self.ChangeChannel.clicked.connect(change_channel)
